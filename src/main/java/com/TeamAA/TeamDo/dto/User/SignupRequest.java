...
public class SignupRequest {
    ...
    //사이즈 처리
    @Size(min = 6, max = 30, message = "아이디는 6자리 이상, 30자리 미만으로 입력해야 합니다.", groups = SizeCheck.class)
    @Schema(description = "회원 아이디", example = "user123")
    private String id;

    @NotBlank(message = "비밀번호를 입력해주세요.", groups = BlankCheck.class)
    @Size(min = 5, max = 16, message = "비밀번호는 5자리 이상, 16자리 미만으로 입력해야 합니다.", groups = SizeCheck.class)
    @Schema(description = "회원 비밀번호", example = "pass1234")
    private String password;

    @NotBlank(message = "이메일을 입력해주세요.", groups = BlankCheck.class)
    @Size(max = 30, message = "이메일은 30자리 미만으로 입력해야 합니다.", groups = SizeCheck.class)
    @Schema(description = "회원 이메일", example = "user123@test.com")
    private String email;

    @NotBlank(message = "이름을 입력해주세요.", groups = BlankCheck.class)
    @Size(max = 30, message = "올바른 이름을 입력해주세요.", groups = SizeCheck.class)
